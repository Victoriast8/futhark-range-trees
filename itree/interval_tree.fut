...
type opt 'v = #some v -- change the naming to something more meaningful
            | #none
type point = f64
        ...
        in match t
           ...
           case #some t ->
                let (_,cnt) = loop (i,acc) = (0,0) while i > 0 do
                    ...
                    in if !(p == c.0) then
                        let dir = p < c.0
                        let (hits,new_i) = if dir -- change to loops 
                            then (map (\i -> if p < i.0 then 0 else 1) t.1[istart:istart+ilen],new_child_idx c.2)
                            else (map (\i -> if p > i.1 then 0 else 1) t.2[istart:istart+ilen],new_child_idx c.3)
                        let sum = reduce (+) 0 hits
                        in (new_i,(acc + sum))
                    else
                        (-1, acc + ilen)
                in cnt
    ...
    def many [n] (iv : [n]interval) : tree =
        if null iv then #none else
            ...
            let (_,_,res,_) = loop (wrk, wrk_shp, acc, num_of_nodes) = (iv, [(i32.i64 n)], [], 0)
                ...
                let min = sgmScan (\x y -> if y < x then y else x) f64.highest flags (map (.0) wrk)
                let max = sgmScan (\x y -> if y > x then y else x) f64.lowest  flags (map (.1) wrk)
                let mid = map (\i -> min[i] + 0.5 * (max[i] - min[i])) ends -- the "x_centers" of each segment, so to speak

                -- let scm = scatter (map f64.i64 (replicate (length wrk) 0)) begs mid
                --           |> sgmScan (+) 0.0 flags
                ...
                let new_non = (last childL) + (last childR) + (last nons) + num_of_nodes
                let children= map3 (\l r n -> let left = if l>0 then #some (i64.i32 (l + new_non + n)) else #none
                                              let right = if r>0 then #some (i64.i32 (r + l + new_non + n)) else #none
                                              in (left,right)
                              ) childL childR nons

                let current_islice = map2 (\off len -> (i64.i32 off, i64.i32 len)) left_offsets split1
                --let upd_islice =
                let new_acc = acc ++ map3 (\p i (l,r) -> create_node p i l r) mid current_islice children
                ...
                let new_shp = intertwine left_offsets right_length
                
                in (new_wrk, new_shp, new_acc, new_non)
