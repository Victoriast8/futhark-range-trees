...
--     def old_many [n] (iv : [n]interval) : tree =
--         let sortedByStart = sort_by_key (.0) (f64.<=) iv
--         let sortedByEnd   = sort_by_key (.1) (f64.<=) iv
--         let accChilds (i : i32) : i32 = if i <= 0 then 0 else 1
--         -- parameters for loop:
...
--             let ends  = map2 (\b i -> b+i-1) begs wrk_shp -- marks the ends of each segement: ends = [1,4,6]
--             --let ends  = scan (+) 0 wrk_shp |> map (+(-1)) -- same ends computation, but independent from 'begs' calculation
--             let size  = i64.i32 (last ends)
--             let flags = mkFlagArray wrk_shp 0i32 (map (+1) (map i32.i64 (indices wrk_shp))) size

--             -- step 1. create mid values for partition
--             let min = sgmScan (\x y -> if x < y then x else y) f64.highest flags (map (.0) wrk :> [size]f64)
--             let max = sgmScan (\x y -> if x > y then x else y) f64.lowest  flags (map (.1) wrk :> [size]f64)
--             let mid = map (\i -> min[i] + 0.5 * (max[i] - min[i])) ends -- the "x_centers" of each segment, so to speak
...
--                                                 (\t -> t.1.1 >= t.0 && t.0 >= t.1.0)
--                                                 (\t -> t.0 > t.1.1)
--                                                 (0.0,(0.0,0.0)) (wrk_shp,(zip scm (wrk :> [size]interval)))
...
def fix_intervals [n] (f1 : [n]f64) (f2 : [n]f64) : [n]interval =
    fix_random_intervals (map2 (\i j -> (i,j)) f1 f2)

let main =
    itree1D.many [(1.0,2.0), (1.0,5.0), (2.0,5.0), (5.0,9.0), (4.0,10.0), (59.0,100.0)]